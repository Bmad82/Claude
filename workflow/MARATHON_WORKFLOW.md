...
| `HANDOVER.json` | Session-Abschluss-State (Single-Slot) | status-Kopf wird beim nächsten Lauf überschrieben, historie bleibt (append-only) |
| `FEATURE_REQUEST_{kurzname}.md` | aktive Aufträge an Coda | bei STATUS=FERTIG → Rename zu `_ERLEDIGT.md` |
| `SCHALTPLAN_PROJEKT.json` | Projekt-Gedächtnis: Module, Status, Brüche (volle Wahrheit) | am Fenster-Ende pflegen, VOR dem Handover |
...
1. **Konflikt-Check**: existiert `HANDOVER.json` mit STATUS=IN_ARBEIT UND `FEATURE_REQUEST_{kurzname}.md` mit anderem Kurznamen? → neuer Request wird zu `_QUEUED.md`, alter zuerst fertig machen.
...
4. `SCHALTPLAN_PROJEKT.json` lesen (Projekt-Gedächtnis: Module, Status, Brüche), falls vorhanden.
...
11. `HANDOVER.json` neu schreiben mit STATUS-Header (FERTIG | IN_ARBEIT | BLOCKIERT) — Pflicht, ausnahmslos.
12. Bei Verzeichnisänderungen: `REPO_INDEX.md` aktualisieren VOR dem finalen Push.
13. Projekt-Gist aktualisieren (PATCH via `workflow/gist_publisher.py`): STATUS, HANDOVER, SCHALTPLAN, ggf. REPO_INDEX, LESSONS — Supervisor kann nur Gists fetchen, keine Raw-Links.
...
`HANDOVER.json` ist der Single-Slot-Übergabepunkt zwischen Sessions. Inhalt: STATUS-Header (Pipe-Format) + 5-10 Zeilen Prosa über den letzten Stand.

- **STATUS=FERTIG**: Auftrag durch, `FEATURE_REQUEST` ist umbenannt zu `_ERLEDIGT.md`, HANDOVER.json wird beim nächsten Session-Start gelesen (status überschrieben, historie bleibt).
- **STATUS=IN_ARBEIT**: Auftrag läuft über mehrere Sessions, `FEATURE_REQUEST` bleibt unverändert, HANDOVER.json trägt Fortschritt + nächsten Schritt.
- **STATUS=BLOCKIERT**: Blocker dokumentiert in `DECISIONS_PENDING.md`, HANDOVER.json trägt BLOCKIERT-Header.

Chris fetcht über den ZUSAMMENFASSUNG-Button die HANDOVER.json vom Handy und sieht sofort, ob Aktion nötig ist.
...
Status-Header-Beispiel (`HANDOVER.json` Zeile 1):
```
...
Die Supervisor-Instanz (claude.ai Chat-Fenster) kann GitHub-Raw-Links **nicht** fetchen, aber GitHub-Gists **schon**. Daher hält jedes Projekt einen parallelen PUBLIC Gist mit den Briefing-Dateien (STATUS, HANDOVER, SCHALTPLAN, REPO_INDEX, LESSONS).
