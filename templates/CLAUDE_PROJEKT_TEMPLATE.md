...
1. **Konflikt-Check:** Existiert `HANDOVER.json` mit `STATUS: IN_ARBEIT`? UND existiert `FEATURE_REQUEST_{PROJEKT}.md` mit abweichendem Kurznamen? → neuen FEATURE_REQUEST zu `_QUEUED.md` umbenennen, alten Auftrag aus HANDOVER.json rekonstruieren, zuerst fertig machen.
...
| HANDOVER.json | Session-Ende | PFLICHT, ausnahmslos — status-Kopf überschreiben + 1 Historie-Eintrag |
| SCHALTPLAN_PROJEKT.json | am Fenster-Ende, VOR dem Handover | Projekt-Gedächtnis: Module/Status/Brüche aktuell halten, Gestrichenes ('—') sichtbar lassen |
| HUMAN_TESTS.json | sobald ein Mensch-Test auftaucht/abgehakt wird | offene Geräte-/Mikrofon-/Touch-/VRAM-Tests, laufend |
...
- Session-Ende-Checkliste: „Gist-Dateien aktualisiert (STATUS, HANDOVER, SCHALTPLAN, ggf. REPO_INDEX, LESSONS)?"
...
5. `git push` | `$LASTEXITCODE` prüfen | bei Fehler: BLOCKIERT in HANDOVER.json
...
5. Projekt-Gist aktualisieren (PATCH via `workflow/gist_publisher.py` aus Claude-Repo): STATUS, HANDOVER, SCHALTPLAN, ggf. REPO_INDEX, LESSONS
6. HANDOVER.json trägt Gist-URL (oder Verweis auf `GIST_LINK.md`) ein, damit Architekt vom Handy direkt zum Gist springen kann

## Stopp-Regeln
- Kontext < 400k Token → sauber abschließen | SCHALTPLAN_PROJEKT.json pflegen | HANDOVER.json mit STATUS=IN_ARBEIT
- Test-Failure → fixen VOR nächstem Feature
- Blockiert → DECISIONS_PENDING.md | STATUS=BLOCKIERT in HANDOVER.json | nächsten unabhängigen Patch
...
## Steuer-Files: Schaltplan (Gedächtnis) + Handover (Staffelstab) + Human-Tests
JSON ist SSOT, die `.html`-Renderer sind reine Anzeige — nie mischen. Drei Files, klare Rollen:
- `SCHALTPLAN_PROJEKT.json` — **Gedächtnis**. Modul-/Abhängigkeitskarte: Module, Status (IST/TEIL/PLAN/—), Brüche, verworfene Ansätze. Status nur mit `ref`-Beleg. Gestrichenes ('—') bleibt sichtbar. Hier wandert alles DAUERHAFTE rein. Vorlage: `C:\Users\chris\Python\Claude\templates\SCHALTPLAN_PROJEKT_TEMPLATE.json`.
- `HANDOVER.json` — **Staffelstab**. NUR heißer In-Flight-Kontext: `status`-Kopf (wird KOMPLETT überschrieben) + `historie` (append-only, 1 Eintrag pro Lauf) + was-Chris-physisch-tun-muss. Verworfene Ansätze/Brüche NICHT hier ablegen — die gehören in den Schaltplan, sonst startet der Nachfolger sie erneut. Vorlage: `C:\Users\chris\Python\Claude\templates\HANDOVER_TEMPLATE.json`.
- `HUMAN_TESTS.json` — offene Mensch-Tests (Kategorien → Items, Abhak-Status). Vorlage: `C:\Users\chris\Python\Claude\templates\HUMAN_TESTS_TEMPLATE.json`.

Wiedereinstieg liest: **Schaltplan (volle Wahrheit) + die letzten 1-2 Handover-Historie-Einträge.** Am Fenster-Ende: erst Schaltplan pflegen, dann Handover schreiben.
